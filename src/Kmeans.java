...
public class Kmeans {
			...
			while(true) {
			    ...
			    if (cluster1 > cluster2 && cluster1 > cluster3 && cluster1 > cluster4) { // if most of k closest measurements belong in cluster 1 ->
			       // 	testobs1.add(testtimes.get(i).intValue()); // -> sort test measurement in cluster 1
			       	System.out.println("Test measurement: "+(i+1)+ " belongs to cluster: 1 -> High load rate during peak hours");
			       	testcbelongs.set(i , 1);
			    }
				
			    if (cluster2 > cluster1 && cluster2 > cluster3 && cluster2 > cluster4) {
			      	// testobs2.add(testtimes.get(i).intValue());
			      	System.out.println("Test measurement: "+(i+1)+ " belongs to cluster: 2-> Low load rate during night");
			      	testcbelongs.set(i , 2);
			    }
				    
			    if (cluster3 > cluster1 && cluster3 > cluster2 && cluster3 > cluster4) {
			      	// testobs3.add(testtimes.get(i).intValue());
			      	System.out.println("Test measurement: "+(i+1)+" belongs to cluster: 3 -> Disconnection of a line for maintenance");
			      	testcbelongs.set(i , 3);
			    }
				    
			    if (cluster4 > cluster1 && cluster4 > cluster2 && cluster4 > cluster3) {
			      	// testobs4.add(testtimes.get(i).intValue());
			      	System.out.println("Test measurement: "+(i+1) +" belongs to cluster: 4 -> Shut down of generator for maintenance");
	...
	public static void K_clusters() {
		...
		double [][] temp_values = new double[200][18];
	...
	public static void Initialization() {
		...
		int[] index = new int[4]; 
			...
			for(int i=0; i<4; i++){
					...
					for(int i=0; i<4; i++){
						for(int j=0; j<18; j++){
							RandCenter[i][j]= NORMmeasurlist[index[i]][j];	
							//System.out.print(RandCenter[i][j] + "\t");
							
						}
						// System.out.print("\n");
					}
centroids = RandCenter; 
					
					
					// calculate the distance
					
					index1=0; // the number of the flowers in the first cluster
				  	index2=0;
				  	index3=0;
				  	index4=0;
					for(int i=0; i<200; i++)
	...
	public static void GetData() throws ClassNotFoundException, SQLException{
		...
		double[][] analogVlist= new double [200][18];
